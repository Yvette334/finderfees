...
export default function Login() {
  ...
  // Check if there's a tab-specific user already logged in
  // If yes, redirect them. Otherwise allow login (for multi-tab presentations)
  useEffect(() => {
    const checkAuth = async () => {
      try {
        // Check sessionStorage first (tab-specific user)
        ...
        if (sessionUser) {
          try {
            ...
            // Invalid session user, continue to login
          }
        }
        // No tab-specific user, allow login
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user) {
        // Store user in both localStorage and sessionStorage (tab-specific)
        try {
          const userData = {
            id: user.id,
            email: user.email,
          ...
          sessionStorage.setItem('activeUserToken', res.session?.access_token || '')
        } catch (e) {
